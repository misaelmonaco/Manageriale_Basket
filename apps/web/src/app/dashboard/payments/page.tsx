...
import { fromCents, isoDate, toCents, useResourceOptions } from "@/components/resource-manager";
import { Button } from "@/components/ui/button";
import { Card } from "@/components/ui/card";
import { Input } from "@/components/ui/input";
import { apiFetch, clientAuth, createResource, deleteResource, type ApiPage } from "@/lib/api";
...
function canManagePayments() {
  ...
  return ["SUPER_ADMIN", "DIRECTOR"].includes(localStorage.getItem("role") ?? "");
}

function playerLabel(player: Player) {
  return player.user ? `${player.user.firstName} ${player.user.lastName}` : player.id;
}
...
function StatusBadge({ status }: { status: PaymentStatus }) {
  ...
  return (
    <span className="inline-flex items-center gap-2 rounded-md border border-border px-2.5 py-1 text-xs font-medium">
      <span className={paid ? "h-2.5 w-2.5 rounded-full bg-emerald-500" : "h-2.5 w-2.5 rounded-full bg-red-500"} />
      {statusLabel(status)}
    </span>
  );
}
...
export default function PaymentsPage() {
  ...
  const [form, setForm] = useState({ playerId: "", description: "", amount: "", dueDate: "" });
  ...
  async function load() {
    ...
    try {
      ...
      const response = await apiFetch<ApiPage<Payment>>(`/payments${params.size ? `?${params.toString()}` : ""}`, clientAuth());
  ...
  return (
    <>
      <PageHeader title="Payments" description="Track player fees, due dates, and collection status." action={action} />
              ...
              <tbody>
                ...
                {payments.map((payment) => (
                  <tr key={payment.id} className="border-t border-border">
                    <td className="px-4 py-3 font-medium">{playerLabel(payment.player)}</td>
                    ...
                    <td className="px-4 py-3">
                      {canManage && (
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <Button variant="ghost" size="icon" aria-label="Delete payment" onClick={() => void remove(payment)}>
