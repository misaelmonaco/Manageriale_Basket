...
import { ResourceManager, isoDate, useResourceOptions } from "@/components/resource-manager";
...
export default function PlayersPage() {
  ...
  async function changePassword(player: Player) {
    ...
    const password = window.prompt(`New password for ${player.user.firstName} ${player.user.lastName}`);
    if (!password) return;
    try {
  ...
  return (
    <ResourceManager<Player>
      ...
      mapRow={(player, actions) => [
        <Link key="player" className="font-medium text-primary underline-offset-4 hover:underline" href={`/dashboard/players/${player.id}`}>
          {player.user ? `${player.user.firstName} ${player.user.lastName}` : "Unassigned"}
        </Link>,
        player.user?.email ?? "",
        player.team?.name ?? "",
        ...
        <div key="actions" className="flex items-center gap-1">
          {player.user && (
            <Button variant="ghost" size="icon" aria-label="Change password" onClick={() => void changePassword(player)}>
