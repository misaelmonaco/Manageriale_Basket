...
type OrganizationDetail = Organization & {
  ...
  _count: { users: number; teams: number; players: number; coaches: number; matches: number };
  teams: {
    id: string;
    name: string;
    category: string;
...
export default function OrganizationsPage() {
  ...
  async function selectOrganization(organization: Organization) {
    ...
    try {
      const detail = await apiFetch<OrganizationDetail>(`/organizations/${organization.id}`, clientAuth());
      setSelected(detail);
    } catch (error) {
      setDetailError(error instanceof Error ? error.message : "Unable to load organization details.");
    }
  }
    ...
    <>
      <ResourceManager<Organization>
        ...
        fields={[
          ...
          { name: "slug", label: "Slug", required: true, placeholder: "basket-roma" },
        ...
        mapRow={(organization, actions) => [
          ...
          <div key="actions" className="flex items-center gap-1">
            <Button variant="ghost" size="icon" aria-label="Details" onClick={() => void selectOrganization(organization)}>
      ...
      {detailError && <p className="mt-4 text-sm text-primary">{detailError}</p>}
      {selected && (
        <Card className="mt-6 p-4">
          <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
            <div>
              <p className="text-xs uppercase text-muted-foreground">Selected organization</p>
              <h2 className="text-lg font-semibold">{selected.name}</h2>
              <p className="text-sm text-muted-foreground">{selected.slug}</p>
            </div>
            {selectedSlug && <p className="rounded-md bg-muted px-3 py-2 text-sm">Slug active: {selectedSlug}</p>}
          </div>
          <div className="grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-5">
            <p><span className="text-muted-foreground">Users</span><br />{selected._count.users}</p>
            <p><span className="text-muted-foreground">Teams</span><br />{selected._count.teams}</p>
            <p><span className="text-muted-foreground">Players</span><br />{selected._count.players}</p>
            <p><span className="text-muted-foreground">Coaches</span><br />{selected._count.coaches}</p>
            <p><span className="text-muted-foreground">Matches</span><br />{selected._count.matches}</p>
          </div>
