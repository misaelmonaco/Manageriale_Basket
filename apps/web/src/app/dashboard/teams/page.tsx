  ...
  return (
    <ResourceManager<Team>
      ...
      fields={[
        ...
        { name: "category", label: "Category", required: true, placeholder: "Under 18" },
        { name: "season", label: "Season", required: true, placeholder: "2026" },
      ]}
      buildPayload={(values) => ({
      ...
      mapRow={(team, actions) => [
        <Link key="team" className="font-medium text-primary underline-offset-4 hover:underline" href={`/dashboard/teams/${team.id}`}>
          {team.name}
        </Link>,
        team.category,
        team.season,
